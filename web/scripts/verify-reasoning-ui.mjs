...
for (let i = 0; i < 10; i++) {
  const ok = await evaljs(`(() => { const items = [...document.querySelectorAll('.sidebar .list .item')]; const it = items.find(x => (x.querySelector('.item-title')||{}).textContent === '推理token验证'); if (!it) return false; it.click(); return true })()`)
  if (ok) break
  await sleep(500)
}
await sleep(1500)
...
const hasReasoning = parsed.some((r) => r.toLowerCase().includes('reasoning'))
console.log(hasReasoning ? 'REASONING ROW OK' : 'REASONING ROW MISSING')
process.exit(0)
