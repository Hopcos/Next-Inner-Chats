...
for (let i = 0; i < 10; i++) {
  const ok = await evaljs(`(() => { const items = [...document.querySelectorAll('.sidebar .list .item')]; const it = items.find(x => (x.querySelector('.item-title')||{}).textContent === 'subagent用量验证'); if (!it) return false; it.click(); return true })()`)
  if (ok) break
  await sleep(500)
}
await sleep(1500)
...
console.log(hasSub ? 'SUB-AGENT ROWS OK in popover' : 'SUB-AGENT ROWS MISSING')
process.exit(0)
